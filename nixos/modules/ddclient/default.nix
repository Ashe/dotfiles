  ...
  config =
      ...
      active =
        lib.mapAttrs
          (
            ...
            svc
            // {
              domain = if svc.domain != null then svc.domain else config.server.publicDomain;
              subdomains =
                if svc.subdomains != null then
                  svc.subdomains
                else
                  lib.attrNames (lib.filterAttrs (_: s: s.public) config.caddy.services);
            }
          )
          (
            lib.filterAttrs (
    ...
    lib.mkIf (active != { }) {
      ...
      age.secrets = lib.mapAttrs' (
        name: _: lib.nameValuePair "ddclient-${name}" { mode = "0444"; }
      ) active;
      ...
      systemd.services.ddclient-dns-check = {
        ...
        serviceConfig = {
          ExecStart =
            let
              validHosts = lib.concatStringsSep " " (
                lib.concatLists (
                  lib.mapAttrsToList (_: svc: map (sub: "${sub}.${svc.domain}") svc.subdomains) active
                )
              );
      ...
      uptime-kuma.monitors = lib.listToAttrs (
        lib.concatLists (
          lib.mapAttrsToList (
            providerName: svc:
            map (
