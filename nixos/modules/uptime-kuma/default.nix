...

{
  ...
  config = lib.mkIf config.uptime-kuma.enable {
    ...
    age.secrets.uptime-kuma-credentials.mode = lib.mkIf
      (config.agenix.secrets != null && builtins.pathExists "${config.agenix.secrets}/uptime-kuma-credentials.age")
      "0444";

    systemd.services.autokuma = lib.mkIf
      (config.agenix.secrets != null && builtins.pathExists "${config.agenix.secrets}/uptime-kuma-credentials.age")
    {
